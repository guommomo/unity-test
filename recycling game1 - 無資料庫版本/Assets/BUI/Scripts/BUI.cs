...
namespace LukeWaffel.BUI{
	...
	public class BUI : Singleton<BUI> {
		...
		void OpenBox(UIBox box){
			...
			shownBox.transform.FindChild ("Header").GetComponentInChildren<Text> ().text = box.header;
			shownBox.transform.FindChild ("Body").FindChild ("Message").GetComponent<Text> ().text = box.body;

			//This is making a refrence to the Buttons part of the UI Box
			GameObject buttons = shownBox.transform.FindChild ("Body").FindChild ("Buttons").gameObject;
